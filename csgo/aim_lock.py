...
def pid(error_x, error_y, kp, ki, kd):
    # 离散形式PID
    global error_sum_x
    global error_sum_y
    global pre_error_x
    ...
    Dout_x = kd * (error_x - pre_error_x)
    pre_error_x = error_x

    Pout_y = kp * error_y
    error_sum_y += error_y
    Iout_y = ki * error_sum_y
    Dout_y = kd * (error_y - pre_error_y)
    pre_error_y = error_y

    print("error_sum_x: ", error_sum_x, " error_sum_y: ", error_sum_x,
          " pre_error_x: ", pre_error_x, " pre_error_y: ", pre_error_y)
...
def lock(aims, top_x, top_y, len_x, len_y, delta_t, args):
    ...
    aims_copy = [x for x in aims_copy if x[0] in args.lock_choice]  # 都是str类型
    ...
    if len(aims_copy):
        ...
        if args.head_first:  # 0:t_head, 1:t_body, 2:ct_head, 3:ct_body
        ...
        if tag in [args.lock_tag[0], args.lock_tag[2]]:  # head
            # *****************************************************************************
            # 算法1：up主的方法
            ...
            # *****************************************************************************
            if flag:  # 如果是压枪状态就暂时不锁定了，up主新版代码后来这里其实删掉了
                return
        elif tag in [args.lock_tag[1], args.lock_tag[3]]:  # body
            # *****************************************************************************
            # 算法1：up主的方法
        ...
        # *****************************************************************************
        # 算法2:bo的方法: 平滑移动系数，除以2后跟up主的效果基本类似！
        rel_x = int(rel_x / 2)
        rel_y = int(rel_y / 2)
        # *****************************************************************************
        ...
        if abs(rel_x) < 5 and abs(rel_y) < 5:  # for debug
            print('locked!!!')

    else:  # len(aims_copy) == 0，即没有要锁定的目标时,PID清0
        if args.lock_strategy == 'pid':
            reset_pid_error()
